    ...
    with torch.no_grad():
        ...
        for data in eval_dl:
            ...
            for user_id, label, score in zip(user_ids, labels, scores):
                ...
                score = score[1:]  #计算该用户和每个item的相似度

                top_scores, top_indices = torch.topk(score, k=10)
                
                recommendations[user_id] = top_indices.cpu().numpy() + 1  # +1 因为
                #print(user_id,recommendations[user_id])
                recommendations[user_id]=history
        ...
        csv_file_path = os.path.join(save_dir, f'recommendations_new.csv')
        
        # 检查是否是主进程，只在主进程中保存文件
        # if dist.get_rank() == 0:
        #     import json
        #     all_dict = json.load(open('/data2/tencent/MicroLens/Code/VideoRec/SASRec/results/sorted_dict.json'))
            
        #     Log_file.info(f"Saving recommendations to {csv_file_path}")
        #     with open(csv_file_path, 'w', newline='') as csvfile:
        #         csv_writer = csv.writer(csvfile)
        #         # 写入CSV标题行
        #         csv_writer.writerow(['user_id', 'recommended_items'])
        #         # 写入每个用户的推荐结果
        #         for user_id, items in recommendations.items():
        #             items = items.tolist()
        #             items = [int(all_dict[str(x)]) for x in items]
        #             csv_writer.writerow([user_id, items])
