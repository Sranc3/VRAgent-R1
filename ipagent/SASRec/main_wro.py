...
from transformers import CLIPVisionModel, SwinForImageClassification, ViTMAEModel, ViTModel, ViTForImageClassification
...
def train(args, model_dir, Log_file, Log_screen, start_time, local_rank):
    ...
    if 'modal' == args.item_tower or 'text' == args.item_tower:
        if 'roberta-base-en' in args.text_model_load:
            ...
            #text_model_load = os.path.abspath(os.path.join(args.root_model_dir, 'pretrained_models/bert', args.text_model_load))
            text_model_load  =  args.text_model_load
    ...
    if 'modal' == args.item_tower or 'image' == args.item_tower:
        if 'vit-b-32-clip' in args.image_model_load:
            ...
            # vit of clip
            image_model_load = 'google/vit-base-patch16-224'
            ...
            image_model_load = os.path.abspath(os.path.join(args.root_model_dir, 'pretrained_models', args.image_model_load))
            # mae
            image_model_load = args.image_model_load
    ...
    if 'modal' == args.item_tower or 'text' == args.item_tower:
        ...
        # 保存item_content以便后续使用
        # try:
        #     save_path = os.path.join(args.root_data_dir, args.dataset, 'item_content.npy')
        #     np.save(save_path, item_content)
        #     Log_file.info(f'成功保存item_content到: {save_path}')
        # except Exception as e:
        #     Log_file.info(f'保存item_content失败: {str(e)}')
    ...
    # #item_content= 
    # try:
    #     load_path = os.path.join(args.root_data_dir, args.dataset, 'item_content.npy')
    #     if os.path.exists(load_path):
    #         item_content = np.load(load_path)
    #         Log_file.info(f'从{load_path}加载item_content成功，形状为{item_content.shape}')
    # except Exception as e:
    #     Log_file.info(f'加载item_content失败: {str(e)}')
    ...
    if 'epoch' in args.load_ckpt_name:
        ...
        start_epoch = 39
        # ckpt_path = '/data2/tencent/MicroLens/Code/VideoRec/SASRec/checkpoint/checkpoint_MicroLens-100k_pairs_with_cold_text/cpt_v1_sasrec_blocknum_2_tau_0.07_bs_256_ed_512_lr_0.0001_l2_0.1_flrText_5e-05_bert-base-chinese_freeze_165_maxLen_10/epoch-116.pt'
        # ckpt_path = '/data2/tencent/MicroLens/Code/VideoRec/SASRec/checkpoint/checkpoint_MicroLens-100k_pairs_text/cpt_v1_sasrec_blocknum_2_tau_0.07_bs_256_ed_512_lr_0.0001_l2_0.1_flrText_5e-05_bert-base-chinese_freeze_165_maxLen_10/epoch-100.pt'
        #ckpt_path = '/data2/tencent/MicroLens/Code/VideoRec/SASRec/checkpoint/checkpoint_MicroLens-100k_pairs_id/cpt_v1_sasrec_blocknum_2_tau_0.07_bs_512_ed_1024_lr_0.0001_l2_0.1_maxLen_10/epoch-12.pt'
        #ckpt_path = '/data2/tencent/MicroLens/Code/VideoRec/SASRec/checkpoint/checkpoint_MicroLens-100k_pairs_colduser_image/epoch-34.pt'
        
        checkpoint = torch.load(args.load_ckpt_name, map_location=torch.device('cpu'))
    ...
    for ep in range(epoch_left):
        ...
        pre_value = 10
        #print('测试的排序样本集是多大？', item_num)
        #if not need_break and (now_epoch-1) % 1 == 0 and now_epoch > 1:
        if not need_break and now_epoch >= 2 and (now_epoch-1) % 1 == 0:
            max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break = \
                eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, users_history_for_valid, \
                    users_valid, 64, item_num, 'val', is_early_stop, local_rank, args, pop_prob_list, Log_file, item_content, item_id_to_keys)
            if dist.get_rank() == 0:
                ...
                get_test_result = subprocess.run(['python', '/data2/tencent/MicroLens/Code/VideoRec/SASRec/run_text_test.py', '--load_ckpt_name', ckpt_path], capture_output=True, text=True, check=True)
                import ipdb; ipdb.set_trace()
                
                output_lines = get_test_result.stdout.splitlines()
                test_results_lines = []
                # 遍历输出的每一行，提取包含'test_results'的行
                for line in output_lines:
                    if 'test_results' in line:
                        test_results_lines.append(line)
                max_eval_value = test_results_lines
...
def save_recommendations(model, user_history, eval_seq, item_scoring, args, item_num,local_rank):
    """为每个用户生成 top-10 推荐商品列表并保存为 CSV 文件"""
    model.eval()
    recommendations = {}
    
    with torch.no_grad():
        for user_id in eval_seq.keys():
            # 获取用户的序列数据
            input_embs = eval_seq[user_id][-args.max_seq_len:]
            input_embs = torch.LongTensor(input_embs).unsqueeze(0).to(local_rank)
            log_mask = torch.ones(input_embs.shape).to(local_rank)
            ...
            scores = scores[1:]  # 排除 padding item (index 0)
            
            # 获取 top-10 推荐
            _, top_indices = torch.topk(scores, k=10)
            print(top_indices)
            recommendations[user_id] = top_indices.cpu().numpy() + 1  # +1 因为之前减去了 padding item
    ...
    for user_id, items in recommendations.items():
        row = [user_id] + items.tolist()
        csv_data.append(row)
    
    # 创建 DataFrame 并保存
    columns = ['user_id'] + [f'item_{i+1}' for i in range(10)]
    df = pd.DataFrame(csv_data, columns=columns)
    save_path = os.path.join('recommendations.csv')
    # df.to_csv(save_path, index=False)
    
    print(f'推荐结果已保存至: {save_path}')
...
def eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, user_history, users_eval, batch_size, item_num,\
    ...
    # 在评估后保存推荐结果
    #save_recommendations(model, user_history, users_eval, item_scoring, args, item_num,local_rank)
...
def main():
    ...
    # parser.add_argument('--local-rank', type=int, default=0, help='Local rank passed by torch.distributed.launch')
    # import os
