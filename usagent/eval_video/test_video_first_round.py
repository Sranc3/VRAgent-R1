...
first_round_anno = json.load(open('/data2/RL/Logic-RL/eval_video/res_first_round_first_copy.json'))
...
def main():
    ...
    with open(args.json_path, 'r') as f:
        ...
        for d in tqdm(f):
            ...
            d = json.loads(d)
            anno_first_resum = search_user_id(d['user_id'])
            if anno_first_resum: 
                results.append(anno_first_resum)
                continue
            ...
            results.append({'user_id':d['user_id'], 'output': result, 'pred_idx_1': [predicted_index], 'pred_id_in_recommend':d['candidate_video_id'][predicted_index]})
            # print('haha')
            with open('res_first_round.json', 'w') as file_json:
